...
(select distinct
    ...
    first_value(ruc.div_ID)over(partition by rd.REQUEST_ID order by rd.doc_date asc) as division_id,
    first_value(rUC.LAST_NAME||' '||rUC.FIRST_NAME||' '||rUC.MIDDLE_NAME)over(partition by rd.REQUEST_ID order by rd.doc_date asc) as fio_cert,
    first_value(ruc1.div_ID)over(partition by rd.REQUEST_ID order by rd.doc_date asc) as division_id1,
    first_value(rUC1.LAST_NAME||' '||rUC1.FIRST_NAME||' '||rUC1.MIDDLE_NAME)over(partition by rd.REQUEST_ID order by rd.doc_date asc) as fio_cert1,
    first_value(rd.global_state)over(partition by rd.REQUEST_ID order by rd.doc_date asc) as global_state,
    first_value(rd.doc_num)over(partition by rd.REQUEST_ID order by rd.doc_date asc) as doc,
    first_value(rd.doc_name)over(partition by rd.REQUEST_ID order by rd.doc_date asc) as doc_name,
    first_value(rd.doc_date)over(partition by rd.REQUEST_ID order by rd.doc_date asc) as doc_date
    , first_value(regexp_replace(replace(replace(replace(
        ...
        dbms_lob.instr(rd.xml, '<Decision>', 180, 1)+10 ),chr(13),' '),chr(10),' '),chr(9),' '),'( *$)|(^ *)|( {2,})',''))over(partition by rd.REQUEST_ID order by rd.doc_date asc) as doc_txt,
    first_value(regexp_replace(replace(replace(replace(
        ...
        dbms_lob.instr(rd.xml, '<Errors>', 180, 1)+8 ),chr(13),' '),chr(10),' '),chr(9),' '),'( *$)|(^ *)|( {2,})',''))over(partition by rd.REQUEST_ID order by rd.doc_date asc) as error_txt
from
    request.request req
    left join REQUEST.REQUEST_DOCUMENT rd on(rd.REQUEST_ID = req.ID and substr(rd.doc_subtype_code, 7) like '025' and rd.global_state in(0,2))
...
select distinct
    ...
    to_char(q.uch_date_create, 'YYYY-MM-DD') as "дата создания",
