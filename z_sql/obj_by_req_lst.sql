...
select distinct
    ...
    replace(replace(dbms_lob.substr(r.name, 4000, 1), chr(10) , ' '), chr(13), '') as "наименование ОН",
    replace(replace(dbms_lob.substr(r.note_long, 4000, 1), chr(10) , ' '), chr(13), '') as "Примечание у ОН",
...
where
    ...
    and o.obj_kind_id in(5,22,23,4,14,20,21,27,28)
...
select distinct
    ...
    '' as "статус ОН",
    to_char(rno.sub_type) as "тип ОН",
...
select distinct
    ...
    '' as "Дата постановки", '' as "Дата снятия",
    to_char(rpon.sub_type) as "статус ОН",
    to_char(rpon.type) as "тип ОН",
    to_char(rpon.designation) as "назначение ОКС",
    '' as "наименование ОН",
    '' as "Примечание у ОН",
    to_char(rpon.cond_cad_num) as "категория ЗУ",
    '' as "РИ по док",
    '' as area,
    '' as zu_area_type,
    rpon.note as "адрес"
